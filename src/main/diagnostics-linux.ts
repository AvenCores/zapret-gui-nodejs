...
import { getLinuxNfqwsPath, isNfqwsRunning, checkNfqwsLaunchable } from './linux/service'
...
async function checkNfqwsDepsRow(dataDir: string): Promise<DiagnosticCheck | null> {
  ...
  const deps = await checkNfqwsLaunchable(p).catch(() => ({ ok: true, list: '', hint: '' }))
  if (deps.ok) {
    return {
      id: 'nfqwsDeps',
  ...
  return {
    ...
    labelKey: 'diag.nfqwsDeps',
    level: 'fail',
    detail: `${deps.list} — fix: ${deps.hint}`,
    detailKey: 'diag.detail.nfqwsDepsFail',
    detailParams: { list: deps.list, hint: deps.hint }
  }
}
