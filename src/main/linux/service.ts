 ...
 *
 * The `daemon` branch runs nfqws in the FOREGROUND via `exec` (no
 * `--daemon`): the runner is the supervised main process for systemd
 * `Type=simple` as well as runit/s6/dinit. Callers must pass a foreground
 * argv (the direct elevated start inside the install batch uses its own
 * `--daemon` argv so the batch itself can return).
 */
export function buildRunnerScript(opts: {
  ...
  // Foreground argv lines for the bash array (one token per line).
  const argvLines = opts.nfqwsArgv.map((a) => `    ${q(a)}`)
  return [
    '#!/usr/bin/env bash',
    ...
    `    cd ${q(opts.binDir)} || exit 1`,
    '    # Foreground on purpose: under systemd (Type=simple) and runit/s6/dinit',
    '    # the runner IS the supervised main process — a daemonizing double-fork',
    '    # would exit immediately and the unit could never stay active (plus',
    '    # systemd would reap the orphaned daemon from the cgroup on stop).',
    '    # exec keeps signals (SIGTERM on systemctl stop) working.',
    '    NFQWS_ARGV=(',
    ...argvLines,
    '    )',
    '    if [ "$(id -u)" -eq 0 ]; then exec "$NFQWS" "${NFQWS_ARGV[@]}"; fi',
    '    if command -v sudo >/dev/null 2>&1; then exec sudo "$NFQWS" "${NFQWS_ARGV[@]}"; fi',
    '    if command -v doas >/dev/null 2>&1; then exec doas "$NFQWS" "${NFQWS_ARGV[@]}"; fi',
    '    exec "$NFQWS" "${NFQWS_ARGV[@]}"',
    '    ;;',
    '  kill)',
    '    log_start kill',
    '    elevate pkill -x nfqws 2>/dev/null || true',
...
/**
 * Whether a generated runner is fresh: has the runner-owned log, a working
 * `--uid`/`--user` pin, and a FOREGROUND nfqws launch. Stale when it:
 * - lacks the log (pre-logging template), or
 * - lacks the pin (nfqws drops to UID 2147483647), or pins root
 *   (`--uid=0:0` — still `EACCES`: `dropcaps()` strips `CAP_DAC_OVERRIDE`,
 *   so even uid 0 cannot traverse a `700` home dir), or
 * - launches nfqws with `--daemon`: the double-fork exits immediately, so a
 *   `Type=simple` unit (and runit/s6/dinit supervision) can never stay
 *   active — systemd reports `inactive/dead` and reaps the orphan.
 * All fail with `cannot access hostlist file ... Permission denied` or a
 * dead unit on lists under `$HOME`. Pure — unit tested.
 */
export function isRunnerFresh(content: string): boolean {
  if (!content.includes('RUN_LOG=')) return false
  // Quoted form included: the runner single-quotes every argv token.
  if (/(^|[\s'"])--daemon([\s'"]|$)/m.test(content)) return false
...
export async function rebuildRunnerFromConf(dataDir: string): Promise<string | null> {
  try {
    ...
    const nfqwsArgv = buildNfqwsArgv(parsed, { binDir, listsDir, daemon: false, runUid: resolveDataOwnerUid(dataDir) })
    return writeRunner(
      dataDir,
      buildRunnerScript({
        nfqwsPath,
...
export async function installLinuxStrategy(
  ...
  const runUid = resolveDataOwnerUid(dataDir)
  // Runner argv is FOREGROUND (the runner is the supervised main process —
  // see buildRunnerScript); the direct batch start below needs --daemon so
  // the batch itself can return while the daemon survives it.
  const runnerArgv = buildNfqwsArgv(parsed, { binDir, listsDir, daemon: false, runUid })
  const batchArgv = buildNfqwsArgv(parsed, { binDir, listsDir, daemon: true, runUid })
  ...
  const missingRefs = missingFiles(extractNfqwsFileRefs(runnerArgv))
  ...
  steps.push({ kind: 'exec', file: nfqwsPath, args: batchArgv, label: 'nfqws --daemon' })
